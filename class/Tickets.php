...
class Tickets extends Database {  
	...
	public function showTickets(){
		...
		while( $ticket = mysqli_fetch_assoc($result) ) {		
			...
			if($ticket['resolved'] == 0)	{
				...
				$status = '<span class="label label-danger">Cerrado</span>';
			}	
			$title = $ticket['title'];
			...
			$ticketRows[] = $status;
			$ticketRows[] = '<a href="view_ticket.php?id='.$ticket["uniqid"].'" class="btn btn-success btn-xs update">Ver ticket</a>';	
			$ticketRows[] = '<button type="button" name="update" id="'.$ticket["id"].'" class="btn btn-warning btn-xs update" '.$disbaled.'>Editar</button>';
			$ticketRows[] = '<button type="button" name="delete" id="'.$ticket["id"].'" class="btn btn-danger btn-xs delete"  '.$disbaled.'>Cerrar</button>';
	...
	public function getRepliedTitle($title) {
		$title = $title.'<span class="answered">Contestado</span>';
		return $title; 		
	}
