$(document).ready(function() {        
	...
	$(document).on('click', '.update', function(){
		...
		$.ajax({
			...
			success:function(data){
				...
				$('.modal-title').html("<i class='fa fa-plus'></i> Editar Departamento");
	...
	$('#addDepartment').click(function(){
		...
		$('.modal-title').html("<i class='fa fa-plus'></i> Añadir Departamento");
	...
	$(document).on('click', '.delete', function(){
		...
		if(confirm("estas seguro de borrar el departamento?")) {
			$.ajax({
				url:"department_action.php",
				method:"POST",
				data:{departmentId:departmentId, action:action},
